...
data_selection = 1; % 1. flower  |　2. toy
if data_selection==1
    data_name = 'flower';
else
    data_name = 'toy'; 
...
load([load_path 'msi_segments_' data_name '.mat']); 
...
for i_sr = [1:1:sr_length]
    ...
    % RMSE_i        = norm(X_ob(Ind)-X_gt(Ind),'fro');
    % RelErr_list(kkk,i_sr)     = RMSE_i/norm(X_gt(Ind),'fro');
    ...
    if EN_CPWOPT
        ...
        % X_cpwopt(ob_ind_int) = X_gt_std(ob_ind_int);
        % X_cpwopt = double(X_cpwopt);
        % Result_std_list_final{kkk,i_sr} = X_cpwopt;
        % X_cpwopt_rtd = X_cpwopt;
